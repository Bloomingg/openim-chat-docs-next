---
title: "好友概览"
description: "OpenIM Platform API 好友模块概览，覆盖好友关系、好友申请、黑名单和好友资料维护。"
product: "platform-api"
context: "chat/platform-api/v3"
template: "overview"
status: "published"
lastUpdated: "2026-07-07"
version: "v3"
sourcePath: "/docs/chat/platform-api/v3/friend/overview"
---

好友模块用于维护用户之间的好友关系、好友申请流程和黑名单数据。所有接口都应由可信后端根据业务规则调用。

## 能力范围

| 能力 | 说明 |
| ---- | ---- |
| 好友关系 | 添加、删除、导入、校验好友关系，并读取好友 ID 或好友资料。 |
| 好友申请 | 查询收到或发出的申请，处理好友申请和查询指定申请。 |
| 黑名单 | 加入黑名单、移除黑名单，以及分页或批量查询黑名单数据。 |
| 资料维护 | 设置好友备注、更新好友扩展信息。 |
| 未处理计数 | 获取当前用户未处理好友申请数量。 |

## 常用接口

- [发送好友申请](/docs/chat/platform-api/v3/friend/managing-friend-requests/apply-to-add-friend)
- [处理好友申请](/docs/chat/platform-api/v3/friend/managing-friend-requests/respond-friend-apply)
- [获取好友列表](/docs/chat/platform-api/v3/friend/listing-friends/list-friends)
- [加入黑名单](/docs/chat/platform-api/v3/friend/blacklist/add-black)
- [获取未处理好友申请数量](/docs/chat/platform-api/v3/friend/managing-friend-requests/get-self-unhandled-apply-count)

## 接入建议

好友关系通常需要结合业务侧隐私、风控和通知策略处理，后端应在调用前完成权限校验。

黑名单会影响用户间互动能力，建议记录操作来源和操作人，便于审计。

## 相关页面

- [接入准备](/docs/chat/platform-api/v3/prepare-to-use-api)
- [错误码](/docs/chat/platform-api/v3/error-codes)
