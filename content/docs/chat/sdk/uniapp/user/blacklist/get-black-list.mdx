...
Use `getBlackList()` in new code in both public and commercial editions. Commercial builds also retain `getBlacks()` as a compatibility entry, and its wrapper field is named `blacks`:
...
Do not mix the `blackUsers` and `blacks` result shapes. Existing commercial integrations can keep `getBlacks()`, but they should not query both snapshots; new cross-edition code should prefer `getBlackList()` and `blackUsers`.
