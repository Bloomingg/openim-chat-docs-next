...
`OpenIMUserInfo` fields include `userID`, `nickname`, `faceURL`, `ex`, optional `createTime`, and optional `globalRecvMsgOpt`. `attachedInfo` is a <span className="enterprise-field-badge">Commercial field</span>; check it for absence and parse it only according to a confirmed commercial contract.
...
| Set how other users can add the current account | [Set friend request permissions](/sdk/uniapp/user/profile/set-friend-add-permission) |
