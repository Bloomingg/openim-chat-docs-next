...
description: 'Set the account-wide globalRecvMsgOpt profile field.'
product: 'sdk'
...
`globalRecvMsgOpt` defines the account's default message-reception and notification policy. It is not an ordinary nickname, avatar, or display-profile field.
...
Promise success means that the update request completed, not that `onSelfInfoUpdated` has arrived. See [Update your profile](/sdk/uniapp/user/profile/set-self-info) for the event and `getSelfUserInfo()` reconciliation.
