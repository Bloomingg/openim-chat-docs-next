...
OpenIMSDK tokens are issued by a trusted backend. Fetch a fresh token and reauthenticate according to product policy when a token expires or becomes invalid.
