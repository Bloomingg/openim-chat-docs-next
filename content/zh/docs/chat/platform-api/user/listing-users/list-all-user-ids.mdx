...
从 `pagination.pageNumber = 1` 开始读取，并根据 `data.total` 和 `pagination.showNumber` 计算是否还有下一页。需要用户资料时，再用返回的 `userIDs` 调用[获取用户](/platform-api/user/listing-users/get-a-user)。
