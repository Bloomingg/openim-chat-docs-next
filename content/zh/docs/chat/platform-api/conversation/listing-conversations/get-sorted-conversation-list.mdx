...
使用 **获取排序会话列表** 从可信后端调用 OpenIM 会话 REST 接口。按置顶状态和最新消息时间分页获取用户会话列表，并返回未读数和最新消息摘要。 请先在[接入准备](/platform-api/prepare-to-use-api)中配置 API 地址和管理员 Token；接口参数通过请求头和 JSON 请求体传递。
