...
`OpenIMUserInfo` 的字段包括 `userID`、`nickname`、`faceURL`、`ex`、可选 `createTime` 和可选 `globalRecvMsgOpt`。只有 `attachedInfo` 是<span className="enterprise-field-badge">商业版</span>字段；读取前应判空，并且只按已确认的商业协议解析。
...
| 设置其他用户添加当前账号时的处理方式 | [设置加好友权限](/sdk/uniapp/user/profile/set-friend-add-permission) |
