...
`globalRecvMsgOpt` 是账号级默认策略，控制当前账号是否接收消息以及是否允许通知。它不属于昵称、头像等普通展示资料。
...
Promise 成功表示设置请求完成，不等于 `onSelfInfoUpdated` 已经到达。事件监听和 `getSelfUserInfo()` 校准方式见[更新当前用户资料](/sdk/uniapp/user/profile/set-self-info)。
