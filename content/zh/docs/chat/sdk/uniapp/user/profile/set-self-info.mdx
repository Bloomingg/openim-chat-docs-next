...
| `globalRecvMsgOpt` | `OpenIMSetSelfInfoRecvMsgOpt` 或 `null` | 否 | 账号级消息接收方式；应在对应设置流程中单独更新。 |
| `addFriendPermission` <span className="enterprise-field-badge">商业版</span> | `0`、`1`、`2` 或 `null` | 否 | 其他用户尝试添加当前账号时使用的策略。 |
...
`setSelfInfo()` 也承载账号级 `globalRecvMsgOpt`，但不应与普通资料一起保存。消息接收策略见[设置全局消息接收方式](/sdk/uniapp/user/profile/set-global-message-reception)。商业版的 `addFriendPermission` 同样通过该 API 更新，并应放在独立设置流程中，见[设置加好友权限](/sdk/uniapp/user/profile/set-friend-add-permission)。
