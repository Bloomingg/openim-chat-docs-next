...
公共版和商业版的新代码都应优先使用 `getBlackList()`。商业版还保留 `getBlacks()` 兼容入口，其包装字段名是 `blacks`：
...
两个入口不要混用返回字段。已有商业版集成可以继续使用 `getBlacks()`，但不要同时查询两份快照；新写的跨版本代码应统一使用 `getBlackList()` 和 `blackUsers`。
